...
export function SponsorGrid({
  ...
  const cellCount = isMobile ? 150 : totalCells;
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <div
        ...
        style={{
          gridTemplateColumns: "repeat(auto-fill, minmax(min(100px, 25vw), 1fr))",
          gridAutoRows: "min(100px, 25vw)",
          gridAutoFlow: "dense",
        }}
      >
        {cells.map(({ position, sponsor }) => (
          <button
            ...
            className={cn(
              ...
              hoveredCell === position &&
                "md:scale-105 z-10 shadow-lg border-white/40 bg-white/10"
            )}
            aria-label={
              sponsor
          ...
          >
            {sponsor ? (
              <div className="flex flex-col items-center justify-center w-full h-full p-1 sm:p-1 gap-0.5 sm:gap-0.5">
                {sponsor.logoUrl ? (
                  <div className={cn("relative w-full h-12 sm:h-10 md:h-8 shrink-0", showGrayscale && "grayscale")}>
                    <Image
                      src={sponsor.logoUrl}
                      alt={sponsor.name}
                      fill
                ...
                <span className="text-[10px] sm:text-[9px] md:text-[8px] font-semibold text-white/80 text-center line-clamp-1 w-full px-0.5">
                  {sponsor.name}
                </span>
                {sponsor.slogan && (
                  <span className="text-[9px] sm:text-[8px] md:text-[7px] text-white/50 text-center line-clamp-1 w-full px-0.5">
                    {sponsor.slogan}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center justify-center w-full h-full opacity-0 hover:opacity-50 transition-opacity">
                <Plus className="w-6 h-6 md:w-4 md:h-4 text-white/40" />
              </div>
            )}

            {/* Tooltip on hover (desktop only) */}
            {hoveredCell === position && (
              <div className="hidden md:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-black/90 text-white text-xs rounded-md shadow-lg border border-white/20 whitespace-nowrap z-50 pointer-events-none">
                {sponsor ? (
                  <>
                    {sponsor.name}
                    <br />
